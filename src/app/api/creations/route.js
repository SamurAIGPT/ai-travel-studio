    ...
    try {
      ...
      if (pollRes.ok) {
        ...
        if (state === "completed" || state === "succeeded") {
          ...
          if (imageUrl) {
            return prisma.travelStudio.update({
              where: { id: record.id },
...
export async function GET(req) {
  try {
    ...
    // Single record fetch
    if (id) {
      let record = await prisma.travelStudio.findFirst({
    ...
    // All records
    let records = await prisma.travelStudio.findMany({
...
export async function DELETE(req) {
  try {
    ...
    const record = await prisma.travelStudio.findFirst({
      where: { id, userId: session.user.id },
    });
    if (!record) {
      return NextResponse.json({ error: "Not found" }, { status: 404 });
    ...
    await prisma.travelStudio.delete({ where: { id } });
